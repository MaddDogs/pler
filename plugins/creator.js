...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨gaarasetyawan@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/gaaraofc`, `nothing`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Developer Bot `, `📵 Don't spam/call me `, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/gaaraofc`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
